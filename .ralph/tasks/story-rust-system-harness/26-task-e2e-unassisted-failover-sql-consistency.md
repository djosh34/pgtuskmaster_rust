...
- [x] BDD features pass (covered by `make test`). (`.ralph/evidence/26-task-e2e-unassisted-failover-sql-consistency/make-test.log`)
</acceptance_criteria>
...
- Track 11: verified required gate order from `Makefile` remains `make check` -> `make test` -> `make test-long` -> `make lint`.
