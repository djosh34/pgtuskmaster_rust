...
- [ ] `make test` excludes only tests with evidence-backed runtime >= 3 minutes and remains the default frequent-run gate.
- [ ] `make test-long` runs only tests with evidence-backed runtime >= 3 minutes and prints a warning to move tests back to regular flow when shortened.
- [ ] Under current evidence, `make test-long` includes exactly `ha::e2e_multi_node::e2e_multi_node_stress_no_quorum_fencing_with_concurrent_sql`.
- [ ] Under current evidence, planned switchover and unassisted failover stress tests run under `make test` (not `make test-long`).
...
- Find any accidental legacy make-target spellings (precise):
  - Disallowed: `make test*` where `*` is not empty and not `-long`:
    - `rg -n -P "make\\s+test(?!-long\\b)\\S+" --glob '!.ralph/progress/**' --glob '!.ralph/archive/**'`
  - Disallowed: `make test-long*` where `*` is not empty:
    - `rg -n -P "make\\s+test-long\\S+" --glob '!.ralph/progress/**' --glob '!.ralph/archive/**'`
  - `rg -n "\\.PHONY:" Makefile`
  - `rg -n "^\\s*(test|test-long):" Makefile`
- Find duplicated/contradictory gate lists (common drift pattern):
  - `rg -n "all BDD features pass" --glob '!.ralph/progress/**' --glob '!.ralph/archive/**'`
...
- Confirm current `Makefile:test` and `Makefile:test-long` behavior matches the ultra-long policy defined in step 0 (it currently does not).
- Update Makefile so:
...
- Canonical gate list in the workflow template:
  ...
  - `make test`
  - `make lint`
  - `make test-long` (last; intentionally ultra-long)
...
- Primary normalization targets:
  1. Replace checklist items like `` `make test` — all BDD features pass `` with wording that does **not** imply a second test group, e.g.:
...
- Mechanically verify after edits:
  - `rg -n "all BDD features pass" .ralph/tasks --glob '!.ralph/progress/**' --glob '!.ralph/archive/**'` returns 0 hits.
...
- Text-level verification (must be clean, excluding progress/archive):
  - `rg -n -P "make\\s+test(?!-long\\b)\\S+" --glob '!.ralph/progress/**' --glob '!.ralph/archive/**'` returns 0 hits.
  - `rg -n -P "make\\s+test-long\\S+" --glob '!.ralph/progress/**' --glob '!.ralph/archive/**'` returns 0 hits.
