...
<acceptance_criteria>
- [x] Run full suite in order: `make check`, `make test`, `make lint`, `make test-bdd`.
...
- Extract BDD failure lines from `make-test-bdd.log` with `rg -n "FAILED|failures|error"`; if none, write explicit `not found`.
