...
**Execution contract for this task:** The HA E2E coverage added here must remain safe under parallel execution. Do not serialize the suite to avoid runtime-binary or pid-management conflicts. The task must preserve or improve nextest-friendly execution with one built runtime binary reused across isolated test namespaces.
...
- If this task uses task 02 hard-kill semantics, they must mean honest OS-level abrupt death and must not rely on `pg_ctl stop -m immediate` being "close enough".
