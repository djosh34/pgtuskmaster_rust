...
Tests in `src/runtime/node.rs` verify that DCS authority overrides local `postgresql.auto.conf` on resume, and that local replica state is rejected when DCS authority is absent.
...
Tests in `src/postgres_managed.rs` verify:

- authoritative managed `pgtm.postgresql.conf` output with managed HBA and ident paths and without `archive_mode`, `archive_command`, or `restore_command`
- signal-file updates for primary, replica, and recovery intents
- managed standby-passfile writes and stale passfile removal on primary start
- `postgresql.auto.conf` quarantine to `pgtm.unmanaged.postgresql.auto.conf`
- managed TLS-file writes for user-supplied identity
- real clone start quarantine of stale auto-conf and signal files
- replica-state readback from managed files
- rejection of conflicting signal files

Tests in `src/runtime/node.rs` verify:

- DCS authority over local auto-conf during resume intent selection
