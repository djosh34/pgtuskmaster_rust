...
- Operator-supplied PostgreSQL TLS material is copied into managed files under `PGDATA`; production certificates are not generated by pgtuskmaster.
...
- `pg_basebackup` no longer writes recovery settings for pgtuskmaster. Replica follow state is regenerated by pgtuskmaster into `pgtm.postgresql.conf` plus managed `standby.signal`.
