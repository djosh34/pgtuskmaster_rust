...
This chapter makes a cautious argument for why the architecture is designed to reduce split-brain risk and unsafe rejoin risk under the failure modes it is designed to handle. It is not a proof of correctness for all environments. It is a structured claim about what the current implementation is trying to preserve and why its design choices support that goal.

## Main claim

The system is designed to reduce the chance of concurrent primaries and unsafe rejoin by making leadership and recovery decisions depend on current local PostgreSQL evidence, current DCS evidence, explicit trust posture, and explicit lifecycle guards rather than on optimistic one-shot assumptions.
...
- recovery paths such as rewind and base backup are explicit before rejoin is considered safe
- startup planning avoids silently trusting unmanaged local state: it rebuilds managed startup intent from explicit evidence and quarantines unmanaged configuration before PostgreSQL starts
...
- recovery can still fail if rewind or clone prerequisites are not actually present
