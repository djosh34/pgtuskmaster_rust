...
- base backup when rewind is unsafe or unavailable
- rejoin as replica after data and coordination state are coherent

For managed starts after rewind, base backup, or rejoin work, `pgtuskmaster` rebuilds the authoritative PostgreSQL startup surface itself. That means:
...
- process job results and timeouts for rewind or base backup work
...
If no believable recovery leader is available, the runtime becomes much more cautious. A node without a trusted source cannot safely reinvent its history. In that case, waiting or fencing may be safer than trying to rejoin immediately.

## Rewinding versus base backup
...
Trust a recovered member only after both the coordination and local PostgreSQL views agree on its role. A node that has just exited rewind or base backup is not automatically safe to treat as a future primary candidate. It first needs to demonstrate that it can rejoin cleanly, follow the current leader, and publish fresh member state that matches the cluster's current reality.
