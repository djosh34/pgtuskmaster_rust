...
[mdBook context]
- Link naturally to managed PostgreSQL runtime files, managed PostgreSQL configuration, runtime config, and TLS reference pages.
...
- Why the project refuses to invent sensitive configuration but still rewrites runtime-owned files.
- Why reserved GUC keys exist.
- Why TLS files are copied into managed runtime paths instead of invented or ignored.
- Why quarantining postgresql.auto.conf fits the managed boundary.
