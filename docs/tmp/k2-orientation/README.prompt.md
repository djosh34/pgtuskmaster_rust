Write exactly one Markdown page for README.md at the repository root.

Requirements:
- This is the repository-facing entry point, not an mdBook page.
- It must briefly explain what pgtuskmaster is using only the provided raw context.
- It must include a quickstart section for running the local docs and the local cluster examples when that is supported by the provided raw context.
- It must link into the docs set using repository-root relative links.
- It must include the exact license text: All rights reserved 'Joshua Azimullah'.
...
```bash
cargo run --bin pgtuskmasterctl -- --base-url http://127.0.0.1:18080 --output text ha state
```
...
Makefile targets related to docs and validation:
