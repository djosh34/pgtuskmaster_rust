...
| `connect_timeout_s` | integer | // todo: defaults.rs establishes the default `5`, but the gathered parser/schema sources in this lane do not define this numeric validation range. Replace with a source-backed constraint or remove the range claim. | 5 |
...
```toml
password = { content = "inline-secret" }   # inline
password = { path = "/run/secrets/file" }    # file
```

Both forms must be non‑empty. The inline content is redacted in debug output.
...
// todo: this overstates behavior from the gathered sources. `SecretSource` has a custom redacted `Debug`, but this lane did not collect a direct source proving how non-secret `InlineOrPath` values are emitted in operational logs. Keep only the schema shape unless a source-backed statement is added.
...
If `init` is present, `payload_json` must be non‑empty and parseable as both JSON and as a `RuntimeConfig`. Validation recursively checks the embedded config.
...
| Field | Type | Constraints |
|-------|------|-------------|
| `loop_interval_ms` | integer | 1 ≤ value ≤ 86 400 000 |
| `lease_ttl_ms` | integer | 1 ≤ value ≤ 86 400 000 |
...
| Field | Type | Constraints | Default |
|-------|------|-------------|---------|
| `pg_rewind_timeout_ms` | integer | 1 ≤ value ≤ 86 400 000 | 120 000 |
| `bootstrap_timeout_ms` | integer | 1 ≤ value ≤ 86 400 000 | 300 000 |
| `fencing_timeout_ms` | integer | 1 ≤ value ≤ 86 400 000 | 30 000 |
...
| `poll_interval_ms` | integer | 1 ≤ value ≤ 86 400 000 | 200 |
...
| `max_age_seconds` | integer | > 0 when enabled | 604 800 (7 days) |
| `protect_recent_seconds` | integer | > 0 when enabled | 300 |

// todo: parser validation in the gathered sources requires `max_files`, `max_age_seconds`, and `protect_recent_seconds` to each be greater than zero when cleanup is enabled. It does not add a separate "at least one of max_files or max_age_seconds" rule.
...
# Inline form (redacted on Debug)
key = { content = "inline value" }
```

For secret material, the inline content is redacted in debug output; paths are shown as `path(<display>)`. For non‑secret sources (e.g., `pg_hba`), inline content appears verbatim.
...
// todo: replace this placeholder with either a real mermaid diagram or remove it before publishing.
