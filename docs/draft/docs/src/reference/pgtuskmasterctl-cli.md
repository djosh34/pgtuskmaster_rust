...
**0**: Success  
**1**: Configuration error (pgtuskmaster node binary)  
...
// todo: This page documents `pgtuskmasterctl`, but the line for exit code `1` is described as a node-binary configuration error and is not supported by the reviewed CLI source set for `pgtuskmasterctl`.
