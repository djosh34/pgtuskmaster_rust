...
// todo: The idempotent/destructive classification in this section is an interpretation from lowering/dispatch behavior, not an explicit contract in the requested sources. Keep the language clearly inferential.

Idempotent decisions can be emitted repeatedly without harm. The engine reissues them while conditions persist. More invasive decisions lower into process jobs that modify data directories, promote instances, or terminate processes; these are dispatched once and guarded by the phase machine.
...
The engine prefers rewind when possible because it is faster and less resource-intensive than basebackup. It selects strategies in this order:

1. **Rewind** when an available primary member exists and timeline mismatches are detected
2. **BaseBackup** when rewind fails or when no previous replica state exists
3. **Bootstrap** when no leader is available and the data directory must be initialized from scratch

// todo: This ordered list is too strong as written. The requested sources support rewind and base-backup fallback paths, but this exact narrative ordering and the "timeline mismatches" phrasing need tightening to the actual decision predicates.

The decision factors are:
- `available_primary_member_id`: a healthy member recorded in DCS
- `rewind_required`: true when local timeline differs from leader's timeline
- Process activity: a rewind job failure triggers fallback to basebackup

This hierarchy keeps recovery as lightweight as possible while guaranteeing progress even after multiple failures.
