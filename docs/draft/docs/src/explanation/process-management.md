...
[diagram about execution domain boundary, showing HA reconciler emitting intents, process worker materializing them into commands, and subprocess execution with outcome publication]
...
[diagram about startup flow, showing runtime node creating ProcessRuntimePlan, calling ensure_start_paths, and passing plan to process startup]
...
[diagram about intent flow, showing HA reconcile generating ProcessIntent, dispatch converting to ProcessIntentRequest with JobId, worker inbox, and either rejection or materialization]
...
[diagram about preflight safety checks, showing PID existence check, socket lock check, and stale file cleanup]
...
[diagram about logging boundary, showing process worker constructing typed events, sending through LogSender opaque handle, and logging worker rendering to sinks]
