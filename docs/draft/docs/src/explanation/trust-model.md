...
- **NotTrusted**: The DCS transport is unreachable or unreliable. The node retains its last observed cluster snapshot but refuses coordinated actions.
// todo: The current code only explicitly gates `NotTrusted` on etcd reachability loss, not a broader "unreliable" classifier. Tighten this wording to match the implementation.
...
[diagram about DCS mode evaluation showing decision flow from etcd reachability to final mode, **more details on diagram**]
// todo: Replace the placeholder line above with a real mermaid diagram that matches the current evaluation flow and passes docs lint.

## Mode Evaluation Logic

The `evaluate_mode()` function in `src/dcs/state.rs` determines the mode in this strict order:
// todo: Keep code references factual, but do not leave hard-coded line numbers in the final prose.
...
// todo: Remove source line references from the final prose while keeping the default value and behavior accurate.

## HA Decision Gating

The mode directly gates high-availability decisions in `src/ha/decide.rs`:
// todo: Keep the file reference if useful, but remove hard-coded line numbers in the final prose.
