...
// todo: The named determinism test was not sourced from the reviewed files and should not be cited unless verified directly.
...
// todo: The removed "never modifies HA state" claim was too strong because the API controller does write switchover requests into DCS.

## Component Organization

The crate root in `src/lib.rs` exposes public modules and internal modules.
// todo: The previous exact counts and "configuration templating" claim were not supported by the reviewed source.
...
// todo: The removed `WorldSnapshot` name and `ha::lower` description were not sourced from the reviewed files in this batch.
...
rest of the system.
// todo: The earlier async job description and examples were not directly supported by the reviewed files here.
...
// todo: The removed detailed success and failure paths were not confirmed from the reviewed excerpt of `src/ha/decide.rs`.
...
// todo: The removed lease-retention and anti-flapping rationale was interpretation beyond the reviewed source.

## Decision Lowering and Effect Plans

// todo: This section cites `ha::lower`, `HaEffectPlan`, effect categories, and named tests that were not part of the reviewed source set for this batch. Remove or replace it with sourced material.
...
// todo: The `debug_api` description was not backed by the reviewed files in this batch and should be removed unless sourced directly.
...
// todo: The named test `dual_primary_sample_is_rejected` was not verified from the reviewed files and should not be cited.
...
// todo: The removed `WorldSnapshot`, `process::worker`, and optimistic-concurrency claims were not established by the reviewed files.
...
// todo: The removed comparison to reactivity, availability, and the lower effect layer went beyond the directly reviewed evidence.
```
