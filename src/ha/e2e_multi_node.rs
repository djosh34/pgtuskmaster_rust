...
impl ClusterFixture {
    ...
    async fn start_with_backup(
        ...
        let pgbackrest_bin = binaries.pgbackrest.clone().ok_or_else(|| {
            WorkerError::Message("e2e harness returned no pgbackrest binary path".to_string())
        })?;

        Ok(Self {
            _guard: guard,
            postgres_bin: binaries.postgres.clone(),
            initdb_bin: binaries.initdb.clone(),
    ...
    fn namespace_child_dir(&self, relative: impl AsRef<Path>) -> Result<PathBuf, WorkerError> {
        let ns = self._guard.namespace()?;
        Ok(ns.child_dir(relative))
    }

    fn pgbackrest_repo1_path(&self) -> Result<PathBuf, WorkerError> {
        self.namespace_child_dir("pgbackrest/repo1")
    }
    ...
    async fn prepare_external_pgbackrest_repo_with_backup(
        &mut self,
        stanza: &str,
        ...
        let source_data_dir = crate::test_harness::pg16::prepare_pgdata_dir(ns, "external-pgbackrest-source")?;
        // Postgres enforces a strict unix socket path limit (107 bytes on linux). Keep the socket
        // directory short so the e2e namespace name cannot make startup fail.
        let source_socket_dir = ns.child_dir("run/ext-pgbackrest-source");
        let source_log_dir = ns.child_dir("pg16/external-pgbackrest-source/logs");
        let spec = crate::test_harness::pg16::PgInstanceSpec {
            postgres_bin: self.postgres_bin.clone(),
            initdb_bin: self.initdb_bin.clone(),
        ...
        self.record(format!(
            "restore repo prep: start external postgres source port={port}"
        ));
        let mut source = crate::test_harness::pg16::spawn_pg16(spec).await?;

        let proof_token = unique_e2e_token()?;
        let proof_table = "e2e_restore_external_proof";
        let create_sql = format!(
            "CREATE TABLE IF NOT EXISTS {proof_table} (id INTEGER PRIMARY KEY, payload TEXT NOT NULL)"
        );
        let insert_sql = format!(
            "INSERT INTO {proof_table} (id, payload) VALUES (1, '{proof_token}') ON CONFLICT (id) DO UPDATE SET payload = EXCLUDED.payload"
        );
        ha_e2e::util::run_psql_statement(
            self.psql_bin.as_path(),
            port,
        ...
        .await?;
        self.record(format!(
            "restore repo prep: external source seeded proof table={proof_table} payload={proof_token}"
        ));
        ...
        let repo1_path = self.pgbackrest_repo1_path()?;
        fs::create_dir_all(&repo1_path).map_err(|err| {
            WorkerError::Message(format!(
                "restore repo prep: create repo1-path {} failed: {err}",
                repo1_path.display()
            ))
        })?;
        ...
        let mut stanza_create_args = base_args.clone();
        stanza_create_args.push("--online=n".to_string());
        stanza_create_args.push("stanza-create".to_string());
        self.record("restore repo prep: pgbackrest stanza-create");
        ...
        let mut backup_args = base_args;
        backup_args.push("--type=full".to_string());
        backup_args.push("--online=n".to_string());
        backup_args.push("backup".to_string());
        self.record("restore repo prep: pgbackrest backup (offline)");
        let _ = ha_e2e::util::run_command_capture_output(
            "pgbackrest backup",
            self.pgbackrest_bin.as_path(),
            &backup_args,
...
#[tokio::test(flavor = "current_thread")]
async fn e2e_multi_node_restore_takeover_external_repo_converges_cluster(
) -> Result<(), WorkerError> {
    ha_e2e::util::run_with_local_set(async {
    let token = unique_e2e_token()?;
    let stanza = format!(
        "e2e-restore-{}",
        sanitize_component(token.as_str())
    );
    let repo = "1".to_string();

    let backup = ha_e2e::BackupHarnessConfig {
        enabled: true,
        bootstrap_enabled: false,
        takeover_policy: crate::config::BackupTakeoverPolicy::Quarantine,
        recovery_mode: crate::config::BackupRecoveryMode::Default,
        pgbackrest: Some(ha_e2e::PgBackRestHarnessConfig {
            stanza: stanza.clone(),
            repo: repo.clone(),
            repo1_path_rel: "pgbackrest/repo1".to_string(),
            options: ha_e2e::PgBackRestHarnessOptions {
                backup: vec!["--log-level-console=error".to_string()],
                info: vec!["--log-level-console=error".to_string()],
                check: vec!["--log-level-console=error".to_string()],
                restore: vec![
                    "--delta".to_string(),
                    "--force=y".to_string(),
                    "--log-level-console=error".to_string(),
                ],
                archive_push: vec!["--log-level-console=error".to_string()],
                archive_get: vec!["--log-level-console=error".to_string()],
            },
        }),
    };

    let mut fixture =
        ClusterFixture::start_with_backup(3, Some(backup), "ha-e2e-multi-node-restore-takeover")
            .await?;
    let mut phase_history: BTreeMap<String, BTreeSet<String>> = BTreeMap::new();

    let scenario_timeout = Duration::from_secs(600);
    let run_result = match tokio::time::timeout(scenario_timeout, async {
        fixture.record("restore takeover bootstrap: wait for stable primary");
        let bootstrap_primary = fixture
            .wait_for_stable_primary(Duration::from_secs(90), None, 5, &mut phase_history)
            .await?;
        fixture.record(format!(
            "restore takeover bootstrap success: primary={bootstrap_primary}"
        ));

        fixture.record("restore takeover pre-check: seed old cluster marker");
        fixture
            .run_sql_on_node_with_retry(
                &bootstrap_primary,
                "CREATE TABLE IF NOT EXISTS e2e_restore_old_marker (id INTEGER PRIMARY KEY, payload TEXT NOT NULL)",
        ...
        fixture
            .run_sql_on_node_with_retry(
                &bootstrap_primary,
                "INSERT INTO e2e_restore_old_marker (id, payload) VALUES (1, 'before-restore') ON CONFLICT (id) DO UPDATE SET payload = EXCLUDED.payload",
                Duration::from_secs(30),
            )
            .await?;

        fixture.record("restore takeover setup: create external pgbackrest repo + offline backup");
        let proof_token = fixture
            .prepare_external_pgbackrest_repo_with_backup(stanza.as_str())
            .await?;
        fixture.record(format!(
            "restore takeover setup: external backup ready proof_token={proof_token}"
        ));

        let executor_member_id = "node-1";
        let executor_index = fixture.node_index_by_id(executor_member_id).ok_or_else(|| {
            WorkerError::Message(format!(
                "restore takeover scenario missing executor node id {executor_member_id}"
            ))
        })?;

        fixture.record(format!(
            "restore takeover request: POST /restore executor={executor_member_id}"
        ));
        let (_control_node_id, control_client) =
            fixture.cli_api_client_for_node_index(fixture.control_node_index()?)?;
        let accepted = control_client
            .post_restore(
                "e2e-controller".to_string(),
                executor_member_id.to_string(),
                Some("e2e restore takeover".to_string()),
                Some(token.clone()),
            )
            .await
            .map_err(|err| WorkerError::Message(format!("POST /restore failed: {err}")))?;
        if !accepted.accepted {
            return Err(WorkerError::Message(
                "POST /restore returned accepted=false".to_string(),
            ));
        }
        fixture.record(format!(
            "restore takeover accepted: restore_id={}",
            accepted.restore_id
        ));

        let (_exec_id, exec_client) = fixture.cli_api_client_for_node_index(executor_index)?;
        fixture.record("restore takeover monitor: wait for status.phase=completed");
        let restore_deadline = tokio::time::Instant::now() + Duration::from_secs(480);
        let mut last_observation = "none".to_string();
        loop {
            if tokio::time::Instant::now() >= restore_deadline {
                return Err(WorkerError::Message(format!(
                    "timed out waiting for restore completion; last_observation={last_observation}"
                )));
            }
            fixture.ensure_runtime_tasks_healthy().await?;
            let observed = match exec_client.get_restore_status().await {
                Ok(status) => {
                    let phase = status
                        .status
                        .as_ref()
                    ...
                    let request_id = status
                        .request
                        .as_ref()
                        .map(|r| r.restore_id.as_str())
                        .unwrap_or("none");
                    let detail = format!(
                        "restore_id={request_id} phase={phase} heartbeat_stale={} last_error={last_error}",
                        status.derived.heartbeat_stale
                    );
                    if status
                        .status
                        .as_ref()
                        .is_some_and(|s| matches!(s.phase, crate::dcs::state::RestorePhase::Completed))
                    {
                        fixture.record(format!("restore takeover completed: {detail}"));
                        break;
                    }
                    detail
                }
                Err(err) => format!("get_restore_status:error={err}"),
            };
        ...
        fixture.record("restore takeover post-check: wait for stable primary on executor");
        let restored_primary = fixture
            .wait_for_stable_primary(Duration::from_secs(180), None, 5, &mut phase_history)
            .await?;
        if restored_primary != executor_member_id {
            return Err(WorkerError::Message(format!(
                "expected restored executor to become primary, got primary={restored_primary} executor={executor_member_id}"
            )));
        }
        fixture
            .assert_no_dual_primary_window(Duration::from_secs(5))
        ...
        fixture.record("restore takeover proof: verify restored data present on new primary");
        let rows_raw = fixture
            .run_sql_on_node_with_retry(
                executor_member_id,
                "SELECT payload FROM e2e_restore_external_proof ORDER BY id",
                Duration::from_secs(60),
            )
            .await?;
        let rows = ha_e2e::util::parse_psql_rows(rows_raw.as_str());
        let expected_rows = vec![proof_token.clone()];
        if rows != expected_rows {
            return Err(WorkerError::Message(format!(
                "restored proof rows mismatch on primary: expected {:?}, got {:?}",
                expected_rows, rows
            )));
        }

        fixture.record("restore takeover proof: verify old cluster marker is absent");
        let marker_raw = fixture
            .run_sql_on_node_with_retry(
                executor_member_id,
                "SELECT to_regclass('public.e2e_restore_old_marker') IS NULL",
                Duration::from_secs(60),
            )
            .await?;
        let marker_rows = ha_e2e::util::parse_psql_rows(marker_raw.as_str());
        if marker_rows != vec!["t".to_string()] {
            return Err(WorkerError::Message(format!(
                "expected old marker table to be absent after restore, got: {marker_rows:?}"
            )));
        }

        fixture.record("restore takeover convergence: verify replicas observe restored data");
        let replica_ids: Vec<String> = fixture
            .nodes
            .iter()
            .map(|node| node.id.clone())
            .filter(|id| id != executor_member_id)
            .collect();
        for replica_id in replica_ids {
            fixture
                .wait_for_rows_on_node(
                ...
                .await?;
            fixture.record(format!(
                "restore takeover convergence: replica {replica_id} has restored proof row"
            ));
        }

        fixture.record("restore takeover cleanup: DELETE /ha/restore");
        let _ = exec_client
            .delete_restore()
            .await
            .map_err(|err| WorkerError::Message(format!("DELETE /ha/restore failed: {err}")))?;
        let cleared = exec_client
            .get_restore_status()
            .await
            .map_err(|err| WorkerError::Message(format!("GET /ha/restore failed: {err}")))?;
        if cleared.request.is_some() || cleared.status.is_some() {
            return Err(WorkerError::Message(
                "expected restore records cleared after DELETE /ha/restore".to_string(),
            ));
        }
    ...
    .await
    {
        ...
        Err(_) => {
            fixture.record(format!(
                "restore takeover scenario timed out after {}s",
    ...
    let artifact_path = fixture.write_timeline_artifact("ha-e2e-restore-takeover-external-repo");
    let shutdown_result = fixture.shutdown().await;
