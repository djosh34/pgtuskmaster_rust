...
pub(crate) async fn run_command_capture_output(
    label: &str,
    program: &Path,
    args: &[String],
    command_timeout: Duration,
    ...
    if program.as_os_str().is_empty() {
        return Err(WorkerError::Message(format!(
            "{} program path must not be empty",
            label_owned
        )));
    }
    ...
    let mut stdout = child.stdout.take().ok_or_else(|| {
        WorkerError::Message(format!("{} stdout pipe unavailable", label_owned))
    })?;
    let mut stderr = child.stderr.take().ok_or_else(|| {
        WorkerError::Message(format!("{} stderr pipe unavailable", label_owned))
    })?;

    let label_for_stdout = label_owned.clone();
    let stdout_task = tokio::spawn(async move {
        let mut buffer = Vec::new();
        stdout
            .read_to_end(&mut buffer)
            .await
            .map(|_| buffer)
            .map_err(|err| WorkerError::Message(format!("{label_for_stdout} stdout read failed: {err}")))
    });
    let label_for_stderr = label_owned.clone();
    let stderr_task = tokio::spawn(async move {
        let mut buffer = Vec::new();
        stderr
            .read_to_end(&mut buffer)
            .await
            .map(|_| buffer)
            .map_err(|err| WorkerError::Message(format!("{label_for_stderr} stderr read failed: {err}")))
    });
    ...
    let stdout_bytes = stdout_task
        .await
        .map_err(|err| WorkerError::Message(format!("{label_owned} stdout join failed: {err}")))??;
    let stderr_bytes = stderr_task
        .await
        .map_err(|err| WorkerError::Message(format!("{label_owned} stderr join failed: {err}")))??;

    let stdout_text = String::from_utf8(stdout_bytes)
        .map_err(|err| WorkerError::Message(format!("{label_owned} stdout utf8 decode failed: {err}")))?;
    if status.success() {
        return Ok(stdout_text);
    }

    let stderr_text = String::from_utf8(stderr_bytes)
        .map_err(|err| WorkerError::Message(format!("{label_owned} stderr utf8 decode failed: {err}")))?;
    Err(WorkerError::Message(format!(
        "{label_owned} exited unsuccessfully with status {status}; stderr={}; stdout={}",
        stderr_text.trim(),
        stdout_text.trim()
    )))
}
