...
- In task files that use lifecycle markers, keep exactly one terminal marker (`TO BE VERIFIED` or `NOW EXECUTE`) at a time; dual markers create ambiguous handoff state and can misroute execution.
- If a public worker method would take a crate-private snapshot type (for example `SystemSnapshot`), keep method visibility `pub(crate)` or you will trigger private-interface warnings under strict clippy `-D warnings`.
