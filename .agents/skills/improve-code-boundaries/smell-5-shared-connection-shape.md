...
## How to untangle smell 5

1. Find the canonical concept.
2. Search for every parser and renderer of that concept.
3. Comment out one ad-hoc renderer or parser and run `make check`.
4. Replace it with the canonical type.
5. If the canonical type itself is bloated or hybrid, flatten that type first.
6. Delete duplicate fields, duplicate constructors, and duplicate string formats.

## Decision rule for smell 5

If the same concept can be parsed in one place and rendered in one place, do not allow three local versions of it.

If the shared type still stores the same idea twice, flatten the shared type before spreading it further.
